...
namespace BracketFields.UI
{
    /// <summary>
    /// Wizard-style dialog: one unique field at a time (Back / Next), with a list of all
    /// fields on the left for jumping around. Values are written straight onto the
    /// <see cref="BracketField"/> objects; the caller replaces text only on
    /// <see cref="DialogResult.OK"/>. Built in code (no designer file) to match the rest of
    /// the repo's hand-maintained projects.
    /// </summary>
    internal sealed class FillFieldsForm : Form
    {
        private readonly IReadOnlyList<BracketField> _fields;
        private int _current = -1;

        private readonly ListBox _fieldList = new ListBox();
        private readonly Label _progressLabel = new Label();
        private readonly Label _tokenLabel = new Label();
        ...
        public FillFieldsForm(IReadOnlyList<BracketField> fields, string documentName)
        {
            _fields = fields ?? throw new ArgumentNullException(nameof(fields));
            BuildLayout(documentName);

            foreach (BracketField field in _fields) _fieldList.Items.Add(field);
            UpdateFilledLabel();
        }
        ...
        private void ShowField(int index)
        {
            ...
            _current = index;
            BracketField field = _fields[index];

            _progressLabel.Text = $"Field {index + 1} of {_fields.Count}";
            _tokenLabel.Text = field.Token;
        ...
        private void TryFinish()
        {
            ...
            {
                MessageBox.Show(this, "Enter a value for at least one field, or click Cancel.",
        ...
        private void UpdateFilledLabel()
        {
            ...
            _filledLabel.Text = $"{filled} of {_fields.Count} filled";
        }
        ...
        private void BuildLayout(string documentName)
        {
            ...
            Text = "Fill Bracket Fields — " + documentName;
            ...
            // Left: every unique field, ticked once it has a value.
            ...
            // Right: the current field.
            _progressLabel.AutoSize = true;
            _progressLabel.ForeColor = SystemColors.GrayText;

            _tokenLabel.AutoSize = true;
            _tokenLabel.Font = new Font("Segoe UI", 13F, FontStyle.Bold);
            _tokenLabel.UseMnemonic = false;
            _tokenLabel.Margin = new Padding(3, 4, 3, 0);
            ...
            var replaceWithLabel = new Label { Text = "&Replace with:", AutoSize = true };
            ...
            {
                Text = "Every occurrence of the field, brackets included, is replaced. Leave blank to skip it.",
                AutoSize = true,
            ...
            ConfigureButton(_replaceButton, "Replace &All", ReplaceButton_Click);
            ...
            // Enter = Next (Finish on the last field); Esc = Cancel.
