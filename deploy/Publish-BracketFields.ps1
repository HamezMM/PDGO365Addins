<#
.SYNOPSIS
    Builds BracketFields in Release and publishes the ClickOnce deployment into the
    synced SharePoint folder the team installs from.

.DESCRIPTION
    Runs `msbuild /t:Publish` (Release), which produces BracketFields.vsto and
    Application Files\ under PublishDir. No setup.exe: team members install from their
    local synced .vsto with Install-BracketFields.ps1 (a web setup.exe gets 403 from
    SharePoint). Any stale setup.exe left in PublishDir is removed. Also drops PDG-CodeSigning.cer and
    Install-BracketFields.ps1 next to it so a team member has everything in one folder.
...
.PARAMETER PublishDir
    ...
    "...\Peake Design - Documents\SOFTWARE RESOURCES\O365\BracketFields".
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Publish-BracketFields.ps1 -Version 1.1.0.0
#>
[CmdletBinding()]
param(
    [string]$PublishDir = (Join-Path $env:USERPROFILE 'Peake Design Group\Peake Design - Documents\SOFTWARE RESOURCES\O365\BracketFields'),
    [string]$Version
)

$ErrorActionPreference = 'Stop'
$repo    = Split-Path $PSScriptRoot -Parent
$csproj  = Join-Path $repo 'BracketFields\BracketFields.csproj'
...
Copy-Item (Join-Path $PSScriptRoot 'Install-BracketFields.ps1')   $PublishDir -Force
Copy-Item (Join-Path $PSScriptRoot 'Uninstall-BracketFields.ps1') $PublishDir -Force
Copy-Item (Join-Path $PSScriptRoot 'INSTALL-BracketFields.md')  (Join-Path $PublishDir 'INSTALL.md') -Force
...
Next:
  ...
  2. Bump AssemblyVersion in BracketFields\Properties\AssemblyInfo.cs and the
     <ApplicationVersion> in the .csproj to $Version, then commit.
  3. Team members run Install-BracketFields.ps1 from their synced copy of that folder.
"@
