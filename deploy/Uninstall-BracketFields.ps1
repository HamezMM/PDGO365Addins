<#
.SYNOPSIS
    Removes the BracketFields Word add-in for the current user.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Uninstall-BracketFields.ps1
#>
[CmdletBinding()]
param(
    [switch]$RemoveCert
...
$uninstall = Get-ChildItem 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall' |
    ...
    Where-Object { $_.DisplayName -like 'BracketFields*' }
...
if ($uninstall) {
    ...
    Write-Host "No BracketFields ClickOnce install found for this user."
}

# Clear the COM add-in registration if a plain build left one behind.
$addinKey = 'HKCU:\Software\Microsoft\Office\Word\Addins\BracketFields'
