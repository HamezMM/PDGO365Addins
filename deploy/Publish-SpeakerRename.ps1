<#
.SYNOPSIS
    Builds SpeakerRename in Release and publishes the ClickOnce deployment into the
    synced SharePoint folder the team installs from.

.DESCRIPTION
    Runs `msbuild /t:Publish` (Release), which produces SpeakerRename.vsto and
    Application Files\ under PublishDir. No setup.exe: team members install from their
    local synced .vsto with Install-SpeakerRename.ps1 (a web setup.exe gets 403 from
    SharePoint). Any stale setup.exe left in PublishDir is removed. Also drops PDG-CodeSigning.cer and
    Install-SpeakerRename.ps1 next to it so a team member has everything in one folder.

.PARAMETER PublishDir
    Local path of the synced target folder. Default is the standard mount point for
    "...\Peake Design - Documents\SOFTWARE RESOURCES\O365\SpeakerRename".
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Publish-SpeakerRename.ps1 -Version 1.1.0.0
#>
[CmdletBinding()]
param(
    [string]$PublishDir = (Join-Path $env:USERPROFILE 'Peake Design Group\Peake Design - Documents\SOFTWARE RESOURCES\O365\SpeakerRename'),
    [string]$Version
)

$ErrorActionPreference = 'Stop'
$repo    = Split-Path $PSScriptRoot -Parent
$csproj  = Join-Path $repo 'SpeakerRename\SpeakerRename.csproj'
...
Copy-Item (Join-Path $PSScriptRoot 'Install-SpeakerRename.ps1')   $PublishDir -Force
Copy-Item (Join-Path $PSScriptRoot 'Uninstall-SpeakerRename.ps1') $PublishDir -Force
Copy-Item (Join-Path $PSScriptRoot 'INSTALL-SpeakerRename.md')  (Join-Path $PublishDir 'INSTALL.md') -Force
...
Next:
  ...
  2. Bump AssemblyVersion in SpeakerRename\Properties\AssemblyInfo.cs and the
     <ApplicationVersion> in the .csproj to $Version, then commit.
  3. Team members run Install-SpeakerRename.ps1 from their synced copy of that folder.
"@
