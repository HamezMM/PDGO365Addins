<#
.SYNOPSIS
    Installs the BracketFields Word add-in for the current user (no admin rights).
...
.DESCRIPTION
    ...
    2. Runs Office's own VSTO installer (VSTOInstaller.exe /i) on the LOCAL synced
       BracketFields.vsto, so Word picks the add-in up next launch.
...
.PARAMETER Source
    Folder holding PDG-CodeSigning.cer and the published BracketFields.vsto (+ Application
    ...
    RESOURCES\O365\BracketFields" when that can't be determined (e.g. pasted into a console).

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Install-BracketFields.ps1
#>
[CmdletBinding()]
param(
    [string]$Source
...
$DefaultSource = Join-Path $env:USERPROFILE 'Peake Design Group\Peake Design - Documents\SOFTWARE RESOURCES\O365\BracketFields'
...
if (-not (Test-Path $Source)) {
    throw "Folder '$Source' not found. Sync the SharePoint folder (see INSTALL.md) or pass -Source <folder with BracketFields.vsto>."
}
...
$vsto = Join-Path $Source 'BracketFields.vsto'
if (-not (Test-Path $vsto)) {
    throw "BracketFields.vsto not found in '$Source'. Make sure the SharePoint folder has finished syncing, or pass -Source."
}
...
Write-Host "`nDone. Close and reopen Word - the 'Home > Fill [Fields]' button should appear."
