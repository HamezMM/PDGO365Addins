<#
.SYNOPSIS
    Removes the SpeakerRename Word add-in for the current user.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\Uninstall-SpeakerRename.ps1
#>
[CmdletBinding()]
param(
    [switch]$RemoveCert
...
$uninstall = Get-ChildItem 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall' |
    ...
    Where-Object { $_.DisplayName -like 'SpeakerRename*' }
...
if ($uninstall) {
    ...
    Write-Host "No SpeakerRename ClickOnce install found for this user."
}

# Clear the COM add-in registration if a plain build left one behind.
$addinKey = 'HKCU:\Software\Microsoft\Office\Word\Addins\SpeakerRename'
