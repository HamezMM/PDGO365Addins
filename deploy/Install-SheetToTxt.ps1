...
.DESCRIPTION
    ...
    2. Runs Office's own VSTO installer (VSTOInstaller.exe /i) on the LOCAL synced
       SheetToTxt.vsto, so Excel picks the add-in up next launch.
...
.PARAMETER Source
    Folder holding PDG-CodeSigning.cer and the published SheetToTxt.vsto (+ Application
    ...
    RESOURCES\O365\SheetToTxt" when that can't be determined (e.g. pasted into a console).
...
$DefaultSource = Join-Path $env:USERPROFILE 'Peake Design Group\Peake Design - Documents\SOFTWARE RESOURCES\O365\SheetToTxt'
...
if (-not (Test-Path $Source)) {
    throw "Folder '$Source' not found. Sync the SharePoint folder (see INSTALL.md) or pass -Source <folder with SheetToTxt.vsto>."
}
...
$vsto = Join-Path $Source 'SheetToTxt.vsto'
if (-not (Test-Path $vsto)) {
    throw "SheetToTxt.vsto not found in '$Source'. Make sure the SharePoint folder has finished syncing, or pass -Source."
}
